...
- **错题重做**：50 题或 100 题练习结束后，若本轮有错题，可进入一次错题重做（仅限本轮作答）；若重做全部答对，奖励一枚蓝宝石 💎。
- **个人纪录**：记录每种题量的最佳成绩、用时与最高连击，打破纪录会显示「新纪录」。
- **错题本**：答错的题目自动收藏，可逐题重练；重练答对后自动从错题本移除。错题卡片对齐展示算式 / 正确答案 / 你的答案。
- **成就页**：查看皇冠数、蓝宝石数、当前等级、升级所需皇冠数以及全部个人最佳纪录。
...
- SQLite 自建 `ArithmeticDbHelper` / `ArithmeticRepository`（无 Room / KSP），三张表：
  ...
  - `rewards`（皇冠奖励，v2）
  - `records`（个人纪录，v3）
- 纯 Kotlin 逻辑（题目生成、皇冠/蓝宝石判定、称号评价）与 Android 解耦，可在宿主 JVM 上直接单测。
...
| `.\gradlew.bat test` | 运行宿主 JVM 单元测试（题目生成、错题选择、皇冠/蓝宝石判定、称号评价） |
