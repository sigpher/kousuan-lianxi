...
- **混合题型**：加减乘除随机出题，固定混合（无题型选择）。加/减操作数 ≤ 100、和不超 100、减法结果非负；乘法为 1..20 × 1..10（两种顺序均可）。
...
- **趣味化设计**：
  ...
  - 实时计时与结果页「平均每题用时」。
  - 童趣称号与鼓励语（如「口算小状元」）。
- **奖励系统**：全对即获得奖励 —— 10 题全对奖励一朵小黄花 🌸、20 题全对奖励红心 ❤️、50 题全对奖励蓝宝石 💎、100 题全对奖励皇冠 👑；按皇冠数划分等级：青铜 / 白银 / 黄金 / 钻石 / 王者。
- **错题重做**：50 题或 100 题练习结束后，若本轮有错题，可进入一次错题重做（仅限本轮作答），做对即从「查看错题」中删除。
- **个人纪录**：记录每种题量的最佳成绩、用时与最高连击，打破纪录会显示「新纪录」。
- **错题与历史**：「查看错题」会收藏每次答错的题目，可通过错题重练删除；「历史错题」为全量历史记录，只查询不删除。错题卡片对齐展示算式 / 正确答案 / 你的答案。
- **成就页**：查看小黄花、红心、蓝宝石、皇冠数量、当前等级、升级所需皇冠数以及全部个人最佳纪录。

## 技术栈

- Kotlin + AndroidX（Navigation、Lifecycle、RecyclerView、ViewBinding）
- SQLite 自建 `ArithmeticDbHelper` / `ArithmeticRepository`（无 Room / KSP），四张表：
  - `wrong_problems`（查看错题，v1）
  - `rewards`（奖励，v2）
  - `records`（个人纪录，v3）
  - `wrong_history`（历史错题，v4，只增不删）
- 纯 Kotlin 逻辑（题目生成、奖励判定、称号评价）与 Android 解耦，可在宿主 JVM 上直接单测。
...
| `.\gradlew.bat test` | 运行宿主 JVM 单元测试（题目生成、错题选择、奖励判定、称号评价） |
